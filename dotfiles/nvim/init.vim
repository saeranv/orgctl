...
" WARNING: don't use vim-sandwich, conflicts w/ textobj
...
" Set all three timeout vars, making sure ttimeoutlen is neg
" Then control all timeouts w/ timeoutlen
set ttimeout| "         don't modify
set ttimeoutlen=-500| " don't modify
set timeout| "          don't modify
set timeoutlen=400| "   modify this
" SET SHELL PARAMS
set fileformat=unix
set nocompatible
" SET VIM BEHAVIOR
...
set showcmd
set nomodeline| " Disable modelines, causing bugs for somereason
set laststatus=2
" Approximation of default statusline (b/c can't reset this).
...
set backupdir=/tmp
set autowrite
let &shell='/usr/bin/zsh'| " to get vim terminal to recognize bash env
set shellcmdflag=-c
" ------------------------------------------------
set ignorecase| " ignore case when search
set smartcase| "  don't ignore case when search if search has uppercase
set wildignorecase| " ignore case when using wilder for autocomplete
" -------------------------------------
" FOLDMETHODS
" za to toggle fold, zd to remove fold
set foldmethod=indent
" start file with no folds
set nofoldenable
set foldlevel=99
...
" ---------------------------------------------------------------------------#
let @e = ':!python %:p'
let @d = ':!tmux send-keys -t bottom "python %:p" ENTER'
ca resete let @e=':!python %:p'<cr>
ca resetd let @d=':!tmux send-keys -t bottom "python %:p" ENTER'<cr>
nnoremap  <silent><leader>e <esc>:write<cr><esc>:<c-r>e<cr>
nnoremap  <silent><leader>d <esc>:write<cr><esc>:<c-r>d<cr><cr>
ca sete let @e=':'<cr>:<c-f>1h
ca setd let @d=':!tmux send-keys -t bottom "" enter'<cr>:<c-f>9h
...
" find/replace substituded text: 
" <c-r> pastes 0 (yank register) into command 
" NOTE: %s searches globally, s searches current line
" <c-g> w/  selection:     :%s/<selection>/<cursor>/g globally
" <c-g> w/o selection:     :%s/<yiw>/<cursor>/g globally
vnoremap <c-g> y<esc>:%s/<c-r>0//g<left><left>
nnoremap <c-g> yiw:%s/<c-r>0//g<left><left>
...
ca bw bw<cr>
ca bd bdelete<cr>
ca cht echo 'catvim<space> \| catcht<space>'<cr> 
ca catvim !cat $vimf \| grep -C2
ca catcht !cat $(cht vim) \|
" ---------------------------------------------------------------------------#
...
let cfg_init_fpath = '~/.config/nvim'
